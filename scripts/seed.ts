...
async function main() {
  ...
  await Category.insertMany(categories.map(([name, description], i) => ({
    name,
    description,
    slug: name.toLowerCase().replace(/[^a-z0-9]+/g, '-'),
    sortOrder: i + 1
  })));
  await Product.insertMany(products.map(([name, category, image, rentalPrice], i) => ({
    name,
    category,
    slug: name.toLowerCase().replace(/[^a-z0-9]+/g, '-'),
    description: 'Couture-grade ethnic wear curated for special moments.',
    image,
    imageType: 'local',
    rentalPrice,
    deposit: 1000,
