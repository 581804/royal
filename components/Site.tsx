...
export default function Site() {
  ...
  return (
    <main className="rv-page">
      ...
      <section id="royal-list" className="rv-access"><div className="rv-access-image"><div>SAME TRADITIONS<br /><span>NEW STORIES</span><i /></div></div><div className="rv-form"><div className="rv-small-title">E A R L Y &nbsp; A C C E S S</div><h2>Join the Royal List</h2><p>Members receive a one-time 10% discount code for their first order.</p><form onSubmit={submit}><div className="rv-form-row"><input aria-label="Full Name" placeholder="Full Name" required /><input aria-label="Email" placeholder="Email" type="email" required /></div><div className="rv-form-row"><input aria-label="Mobile Number" placeholder="+91   Mobile Number" /><select aria-label="Select an occasion" defaultValue=""><option value="" disabled>Select an occasion</option><option>Wedding</option><option>Reception</option><option>Engagement</option><option>Festival</option></select></div><button type="submit">{sent ? 'YOU’RE ON THE ROYAL LIST' : 'CLAIM MY 10% CODE'} <ChevronRight /></button></form></div><div className="rv-access-side">TRADITIONS<br />TODAY<br /><span>TOMORROW</span><br />ALWAYS</div></section>
      <footer id="contact" className="rv-footer"><div className="rv-footer-main"><div className="rv-brand-foot"><img src="/logo.png" alt="The Royal Vastra" /><p>Tradition for every occasion.</p></div><div><h4>QUICK LINKS</h4><a href="#home">Home</a><a href="#collection">Collections</a><a href="#how">How It Works</a><a href="#about">About Us</a><a href="#blog">Blog</a><a href="#contact">Contact</a></div><div><h4>CONTACT</h4><p><MapPin /> Udaipur, Rajasthan</p><p><Mail /> sale@theroyalvastra.in</p><p>◎ &nbsp; www.theroyalvastra.com</p></div><div><h4>FOLLOW US</h4><div className="rv-social"><Instagram /><Facebook /><span>℘</span><Youtube /></div></div></div><div className="rv-footer-bottom"><span>© 2026 The Royal Vastra. All rights reserved.</span><span>Privacy Policy &nbsp;&nbsp; | &nbsp;&nbsp; Terms &amp; Conditions</span></div></footer>
    </main>
  );
}
